...
**Workaround for `loadfile`.** Compy's `loadfile` does not resolve
relative paths from the project directory, so `main.lua` uses
`loadstring(readfile(name))` as a temporary substitute. Bug report
filed separately; the marker comment in `main.lua` points to the
three lines to revert once the upstream fix lands.
